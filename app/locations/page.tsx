...
export default function LocationsPage() {
  return (
    <>
      ...
      <section className="py-20 md:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <SectionHeading
            ...
            label="Branches"
            title="Our Branches"
            subtitle="All branches closed on federal holidays."
          />
          <AnimateOnScroll animation="stagger-children">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {branches.map((branch) => (
                <div
                  ...
                  className={`bg-white rounded-2xl p-6 transition-shadow ${
                    branch.isHQ ? "shadow-lg ring-1 ring-gold/20 lg:col-span-1" : "shadow-sm hover:shadow-md"
                  }`}
                >
                  <div className="flex items-start justify-between mb-3">
                    <h3 className="text-base font-semibold text-navy">{branch.name}</h3>
                    {branch.isHQ && (
                      <span className="px-2 py-0.5 text-xs font-semibold bg-gold/10 text-gold-dark rounded-full">
                        HQ
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {branch.features.map((f) => (
                      <span
                        key={f}
                        className="px-2 py-1 text-xs font-medium bg-navy/5 text-body rounded-md"
                      >
                        {f}
                      </span>
                    ))}
              ...
              {/* ATM Network Card */}
              <div className="bg-navy grain-overlay rounded-2xl p-6 text-white relative overflow-hidden">
      ...
      <section className="py-20 md:py-28 bg-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <SectionHeading
            variant="minimal"
            title="Contact Us"
            subtitle="Reach the right team for your needs."
          />
          <AnimateOnScroll animation="stagger-children">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl mx-auto">
              {contactInfo.map((item) => (
                <div key={item.label} className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-5">
                  <h3 className="text-sm font-semibold text-navy mb-1">{item.label}</h3>
                  <a
                    href={`tel:${item.number.replace(/[^0-9]/g, "")}`}
                    className="text-xl font-light text-gold-dark hover:text-gold transition-colors"
                  >
                    {item.number}
                  </a>
      ...
      {/* Holidays — inline list without wrapper card */}
      <section className="py-16 md:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <SectionHeading
            variant="minimal"
            title="Holiday Schedule"
            subtitle="All branches are closed on the following holidays."
          />
          <div className="max-w-2xl mx-auto">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
              {holidays.map((holiday) => (
                <div key={holiday} className="flex items-center gap-3 text-sm text-body py-2 border-b border-border/40 last:border-0">
